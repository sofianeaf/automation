...
def locateOnScreen_tuna(image1, image2, image3, image4, grayscale=False,region=None):
    screenshotIm = screenshot(region=region)
    retVal = locate(image1, screenshotIm, grayscale)
    if 'fp' in dir(screenshotIm) and screenshotIm.fp is not None:
        screenshotIm.fp.close() # Screenshots on Windows won't have an fp since they came from ImageGrab, not a file.
        return retVal
    else:
        retVal = locate(image2, screenshotIm, grayscale)
        if 'fp' in dir(screenshotIm) and screenshotIm.fp is not None:
            screenshotIm.fp.close() # Screenshots on Windows won't have an fp since they came from ImageGrab, not a file.
            return retVal
        else:
            retVal = locate(image3, screenshotIm, grayscale)
            if 'fp' in dir(screenshotIm) and screenshotIm.fp is not None:
                screenshotIm.fp.close() # Screenshots on Windows won't have an fp since they came from ImageGrab, not a file.
                return retVal
            else:
...
def locateCenterOnScreen_tuna(image1, image2, image3, image4, grayscale=False, region=None):
    a = locateOnScreen_tuna(image1, image2, image3, image4, grayscale, region)
    if a is None:
        return None
    else:
        return center(a)
