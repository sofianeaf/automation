...
def locateOnScreen2(target, image, grayscale=False,region=None):
    #screenshotIm = screenshot(region=region)
    screenshotIm = target
    retVal = locate(image, screenshotIm, grayscale)
    if 'fp' in dir(screenshotIm) and screenshotIm.fp is not None:
...
def locateCenterOnScreen2(target, image, grayscale=False, region=None):
    a = locateOnScreen2(target, image, grayscale, region)
    if a is not None:
        return center(a)
    else:
        return None
